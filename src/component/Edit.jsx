      ...
      <div className='flex'>
        {
          Contact.map(i => {
            ...
            i.id == location.state.id ? console.log('work'):console.log('not')
            if (i.id == location.state.id) {
              return (<>
                <div className='w-[80%] mb-3 relative flex items-center space-x-7'>
                <div className='text-gray-500 absolute  text-2xl top-0 '>
                    <AiOutlinePlus onClick={()=>nav(-1)} className='rotate-45'/>
                </div>
          <div className='w-[15%] ml-5 relative'>
              <input ref={inputTag} onChange={()=>setImage(inputTag.current.files[0])} type="file" className='hidden' />
              <img ref={editImageTag} className='w-full rounded-full' src={i.photo} />
            ...
            <div className='flex'>
                <button className=' mt-auto mb-5 text-[0.9rem]  px-7 py-2 text-white rounded bg-sky-500'>Edit</button>
            </div>
              </>
